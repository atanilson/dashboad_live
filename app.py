...
protected_marine = gpd.read_file("WDPA_WDOECM_Mar2025_Public_GBR_shp_2.zip")
...
intersected = gpd.read_file("intersected.zip")
...
#Data
uk_countries_df = gpd.read_file("uk_countries_df.zip")
uk_local_authorities_df = gpd.read_file("uk_local_authorities_df.zip")
england_regions_df = gpd.read_file("england_regions_df.zip")
...
def update_chart(df,country, region,lad):
    ...
    #filter
    filtered_df["porpotion_area"] = ((filtered_df["protected_"])/adm_area)*100
...
#protected_marine = gpd.read_file("protected_marine.zip")
